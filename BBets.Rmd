...
for (i in 1:100){
  saveProb[i] = 0
  sampleB=sample(urn,size=numStudents,replace=T)
  repeatB = length(unique(sampleB))
  if (repeatB < 35) {
    saveProb[i] = 1
  }
}
 
probWin = sum(saveProb)/100   
print(paste('prob of win', probWin))
      
expectedReturn = probWin*numBets*1 - (1-probWin)*numBets*1
print(paste('expected win per class', expectedReturn))

```
probability of win = probWin
expected return per class isexpectReturn
